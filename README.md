...
- `update:modelValue`：内容变更时触发，配合 `v-model` 使用可使外部变量始终与编辑器内容保持同步

### BambooEditor 暴露方法（defineExpose）

- `clearDraft()`：清除当前编辑器实例对应的本地草稿，内部基于组件自身的 `editorId` 定位草稿，无需手动传入 ID；适合在用户正常保存成功后调用
...
接入方可通过 `colorPalette` 扩展更多 token；编辑器会立即识别并渲染，展示端需同步补充对应 CSS。
